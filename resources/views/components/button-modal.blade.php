...
<button name="{{ $id }}" data-modal-target="{{ $modal }}" data-modal-toggle="{{ $modal }}" @if(!empty($fonction)) wire:click="{{ $fonction }}('{{ $arg ?? '' }}')" @endif class="text-gray-700 hover:text-white dark:text-white bg-{{ $color ?? 'blue' }}-700 hover:bg-{{ $color ?? 'blue' }}-800 focus:ring-4 focus:outline-none focus:ring-{{ $color ?? 'blue' }}-300 font-medium rounded-lg text-sm px-2 py-2 text-center dark:bg-{{ $color ?? 'blue' }}-600 dark:hover:bg-{{ $color ?? 'blue' }}-700 dark:focus:ring-{{ $color ?? 'blue' }}-800 flex items-center justify-center" type="button" title="{{ $titre ?? '' }}">
    ...
    @elseif (isset($icon) && $icon === 'update')
    <svg class="w-6 h-6 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m14.304 4.844 2.852 2.852M7 7H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-4.5m2.409-9.91a2.017 2.017 0 0 1 0 2.853l-6.844 6.844L8 14l.713-3.565 6.844-6.844a2.015 2.015 0 0 1 2.852 0Z" />
    </svg>
    @elseif(isset($icon) && $icon === 'process')
    <svg class="w-6 h-6 text-white dark:text-white mr-2" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    ...
    @elseif(isset($icon) && $icon ==="sortie")
    <svg class="h-4 w-4 mr-2 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" d="m15 15 6-6m0 0-6-6m6 6H9a6 6 0 0 0 0 12h3" />
    </svg>
    @else
    <svg class="h-4 w-4 mr-2 text-white" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
